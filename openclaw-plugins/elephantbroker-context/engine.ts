...
export class ContextEngineImpl {
  ...
  async dispose(): Promise<void> {
    await this.flushBuffer();
    await this.client.dispose(this.currentSessionKey, this.currentSessionId);
    this.currentSessionKey = "agent:main:main";
    this.currentSessionId = "";
    this.messageBuffer = [];
    // NOTE: Do NOT clear lastTurnMessages here — dispose() is called by OpenClaw
    // BEFORE afterTurn(), so clearing would destroy messages buffered by agent_end.
